...
docker pull sparklyballs/shellcheck"${LINT_ARCH}"

docker run \
	--rm=true -t \
	${MOUNT_OPTIONS} \
	sparklyballs/shellcheck${LINT_ARCH} \
...
docker pull sparklyballs/hadolint
...
docker run \
	...
	hadolint -f checkstyle -c /config/hadolint.yaml /Dockerfile > ${WORKSPACE}/hadolint-result.xml
